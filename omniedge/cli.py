...
def resolve_base_url(args_base_url: Optional[str]) -> str:
    env_base = os.getenv("OMNIEDGE_BASE_URL")
    if args_base_url:
        return args_base_url
    if env_base:
        return env_base
    return "https://api.omniedge.ai"


def resolve_api_key(preset: Optional[str]) -> str:
    env_key = os.getenv("OMNIEDGE_API_KEY")
    if env_key:
        return env_key
    if preset:
        return preset
...
def build_parser() -> argparse.ArgumentParser:
    ...
    set_parser.add_argument("--api-key", help="API key (overrides OMNIEDGE_API_KEY)")
    set_parser.add_argument("--base-url", help="Base URL (overrides OMNIEDGE_BASE_URL)")
